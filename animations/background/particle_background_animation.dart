...
class ParticleBackgroundAnimation extends StatelessWidget {
  final Widget child;
  final bool isRadomBackground;
  final int count;
  final Color color;
  const ParticleBackgroundAnimation(
      {required this.color,
      this.isRadomBackground = false,
      this.count = 40,
      required this.child,
      Key? key})
      : super(key: key);
  ...
  Widget build(BuildContext context) {
    return Stack(
      children: [
        if (isRadomBackground) Positioned.fill(child: AnimatedBackground()),
        Positioned.fill(
            child: Particles(
          color: color,
...
class Particles extends StatefulWidget {
  ...
  const Particles(
      {Key? key, required this.color, required this.numberOfParticles})
      : super(key: key);
...
class ParticleModel {
  ...
  restart({Duration time = Duration.zero}) {
    ...
    tween = MultiTrackTween([
      Track("x").add(
          duration,
          Tween(begin: startPosition.dx, end: startPosition.dx //endPosition.dx
              ),
...
class ParticlePainter extends CustomPainter {
  ...
  ParticlePainter(this.particles, this.time, this.c);
  ...
  void paint(Canvas canvas, Size size) {
    ...
    for (var particle in particles) {
      ...
      Rect rect = Rect.fromCenter(
          center: position,
          width: size.width * 0.1 * particle.size - 10,
          height: size.width * 0.1 * particle.size - 10);
      canvas.drawRect(rect, paint);
    } //position, size.width * 0.2 * particle.size, paint
  }
