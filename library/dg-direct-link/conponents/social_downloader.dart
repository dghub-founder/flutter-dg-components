...
class SocialDownloader {
  ...
  ///
  static Future<List<DGDirectLinkModel>> getLink(
      {required String link, int timeout = 6}) async {
    VideoData? result = await getDirectLink(link: link, timeout: timeout)
        .then((value) => value!);
    ...
    if (result == null || result.links == null) {
      ...
      for (var i = 0; i < result.links!.length; i++) {
        linkList.add(DGDirectLinkModel(
            quality: result.links![i].text!, link: result.links![i].href!));
      }
      return linkList;
    }
  }
  ...
  static Future<VideoData?> getDirectLink(
      {String link = '', int timeout = 6}) async {
    VideoData? res;
